...
export default function Applications({
  ...
  const actionBtnStyle = (variant) => {
    ...
    if (variant === "view") {
      return { ...common, background: isJob ? T.blueLight : T.tealLight, color: isJob ? T.blue : T.tealDark };
    }
    if (variant === "status") {
      return { ...common, background: T.amberLight, color: T.amber };
    }
    if (variant === "shortlist") {
      return { ...common, background: T.greenLight, color: T.green };
    }
    if (variant === "offer") {
      return { ...common, background: T.accentLight, color: T.accentDark };
    }
    return { ...common, background: T.redLight, color: T.red };
  };
  ...
  return (
    <div>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          {isMobile ? (
            <>
              <div
                ...
                style={{
                  ...
                  scrollbarWidth: "none", msOverflowStyle: "none", gap: 12, paddingBottom: 4,
                }}
              >
                <div
                  onClick={() => {
                    ...
                    >
                      <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          ...
                          <div style={{ fontSize: 11, color: T.inkFaint, marginTop: 2 }}>{p.channel === "Internal" ? "Internal Portal" : "Career Portal"}</div>
                        </div>
                        {isSelected && (
                          <div style={{ background: accentColor, color: "#fff", borderRadius: 99, padding: "4px 12px", fontSize: 11, fontWeight: 700, flexShrink: 0 }}>Active</div>
                        )}
                      ...
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 14, gap: 8 }}>
                        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                          <span style={{ fontSize: 11, borderRadius: 99, padding: "3px 10px", fontWeight: 700, background: p.type === "Full-time" ? T.blueLight : T.tealLight, color: p.type === "Full-time" ? T.blue : T.tealDark }}>{p.type}</span>
                          <span style={{ fontSize: 11, borderRadius: 99, padding: "3px 10px", fontWeight: 700, ...(p.status === "Published" ? { background: T.greenLight, color: T.green } : { background: T.amberLight, color: T.amber }) }}>{p.status === "Published" ? "● Live" : "○ Draft"}</span>
                          {p.exp && p.exp !== "—" && (
              ...
              <div style={{ display: "flex", justifyContent: "center", gap: 6, marginTop: 10 }}>
                {[null, ...enrichedPostings.map((p) => p.id)].map((id, i) => (
                  <div
                    ...
                    style={{ width: filterActiveIndex === i ? 20 : 6, height: 6, borderRadius: 99, background: filterActiveIndex === i ? accentColor : T.border, cursor: "pointer", transition: "all 0.2s" }}
                  />
                ))}
              </div>
            </>
            ...
            <div style={{ position: "relative" }}>
              <div style={{ position: "absolute", left: 0, top: 0, bottom: 8, width: 40, zIndex: 2, background: `linear-gradient(to right, ${T.canvas}, transparent)`, pointerEvents: "none", borderRadius: "14px 0 0 14px" }} />
              <div style={{ position: "absolute", right: 0, top: 0, bottom: 8, width: 40, zIndex: 2, background: `linear-gradient(to left, ${T.canvas}, transparent)`, pointerEvents: "none", borderRadius: "0 14px 14px 0" }} />
              <div
                ref={hScroll.ref}
              ...
              >
                ...
                >
                  ...
                  <div style={{ fontSize: 11, color: T.inkFaint, textAlign: "center" }}>{isJob ? jobApplications.length : generalApplications.length} total</div>
                  {!selectedPostingId && (
                ...
                {enrichedPostings.map((p) => {
                  const isSelected = selectedPostingId === p.id;
                  const quals = p.qual ? p.qual.split("+").map((q) => q.trim()).filter(Boolean) : [];
                  const displayCount = isJob ? p.jobAppCount : p.genAppCount;
                  return (
                    <div
                      key={p.id}
                    ...
                    >
                      <div style={{ padding: "14px 16px 10px", borderBottom: `1px solid ${isSelected ? `${accentColor}22` : T.border}`, background: isSelected ? `${accentColor}0d` : T.canvas }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8, marginBottom: 4 }}>
                          <div style={{ fontSize: 14, fontWeight: 800, color: T.ink, lineHeight: 1.3, flex: 1 }}>{p.role}</div>
                          <span style={{ flexShrink: 0, fontSize: 10, fontWeight: 700, borderRadius: 99, padding: "3px 9px", background: p.type === "Full-time" ? T.blueLight : T.tealLight, color: p.type === "Full-time" ? T.blue : T.tealDark, whiteSpace: "nowrap" }}>{p.type}</span>
                        </div>
                        <div style={{ fontSize: 11, color: T.inkFaint }}>{p.channel === "Internal" ? "Internal Portal" : "Career Portal"}</div>
                      </div>
                      <div style={{ padding: "10px 16px", display: "flex", gap: 6, flexWrap: "wrap", borderBottom: `1px solid ${isSelected ? `${accentColor}18` : T.border}` }}>
                        ...
                        <span style={{ fontSize: 11, borderRadius: 6, padding: "3px 8px", fontWeight: 600, ...(p.status === "Published" ? { background: T.greenLight, color: T.green } : { background: T.amberLight, color: T.amber }) }}>{p.status === "Published" ? "● Live" : "○ Draft"}</span>
                      </div>
                      {quals.length > 0 && (
                      ...
                      <div style={{ padding: "10px 16px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <span style={{ fontSize: 11, color: T.inkFaint }}><span style={{ fontWeight: 700, color: isSelected ? accentColor : T.ink, fontSize: 15 }}>{displayCount}</span> applicants</span>
                        {isSelected && <span style={{ fontSize: 10, fontWeight: 700, background: accentColor, color: "#fff", borderRadius: 99, padding: "2px 9px" }}>Selected</span>}
                      </div>
                    </div>
                  );
                })}
      ...
      <div style={{ display: "flex", gap: 0, marginBottom: 20 }}>
        {[
          { key: "job", label: isMobile ? "Job Post" : "Job Post Applications", count: jobCount, accent: T.blue, light: T.blueLight },
          { key: "general", label: isMobile ? "Profiles" : "General Applications (Profiles)", count: genCount, accent: T.tealDark, light: T.tealLight },
        ].map((t, i) => (
          <button
            key={t.key}
      ...
      {isMobile ? (
            ...
            {filtered.map((a, idx) => {
              ...
              return (
                <div
                  ...
                  style={{
                    flexShrink: 0, width: "calc(100% - 24px)", scrollSnapAlign: "center",
                    borderRadius: 20, background: cardBackground, color: "#fff",
                    display: "flex", flexDirection: "column", justifyContent: "space-between",
                    padding: 24, position: "relative", boxShadow: "0 14px 40px rgba(0,0,0,0.25)",
                  ...
                  <div style={{ background: "rgba(255,255,255,0.08)", backdropFilter: "blur(8px)", borderRadius: 14, padding: 16, border: "1px solid rgba(255,255,255,0.12)", display: "flex", flexDirection: "column", gap: 10, marginTop: 16, flex: 1 }}>
                    {[
                      ...
                      <div key={index} style={{ display: "flex", gap: 12, alignItems: "center" }}>
                        ...
                        <div style={{ minWidth: 0, flex: 1 }}>
                          ...
                          <div style={{ fontSize: 13, color: "#fff", fontWeight: 600, marginTop: 1, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{item.value}</div>
                        </div>
                      </div>
                    ))}
            ...
            <div style={{ display: "flex", justifyContent: "center", gap: 6, marginTop: 10, paddingBottom: 8 }}>
              {filtered.map((_, i) => (
                <div
                  ...
                  style={{ width: 8, height: 8, borderRadius: "50%", background: currentCardIndex === i ? T.primary : T.border, cursor: "pointer", transition: "all 0.3s" }}
                />
              ))}
            </div>
          )}
        ...
        <Card>
          <div style={{ padding: "12px 14px", borderBottom: `1px solid ${T.border}`, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
            <Input
        ...
        {selectedApp && (
          <div style={{ overflow: "hidden" }}>
            ...
            <div style={{
              ...
              <div style={{ width: 52, height: 52, borderRadius: 14, background: "rgba(255,255,255,0.15)", backdropFilter: "blur(8px)", border: "1px solid rgba(255,255,255,0.2)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, fontWeight: 800, color: "#fff", flexShrink: 0 }}>
                {selectedApp.name.split(" ").map((n) => n[0]).join("").substring(0, 2).toUpperCase()}
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ padding: "20px 0 0" }}>
              ...
              <div style={{ display: "flex", flexDirection: isMobile ? "column" : "row", justifyContent: "space-between", alignItems: isMobile ? "stretch" : "center", marginTop: 20, borderTop: `1px solid ${T.border}`, paddingTop: 16, gap: 10 }}>
                <a
                  href={selectedApp.resume || "#"}
                  target="_blank"
                  rel="noreferrer"
                  onClick={(e) => { if (!selectedApp.resume) { e.preventDefault(); alert("Resume not available."); } }}
                  style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 6, padding: "9px 16px", borderRadius: 10, background: T.canvas, border: `1.5px solid ${T.border}`, color: T.blue, textDecoration: "none", fontWeight: 700, fontSize: 13 }}
                >
                  📄 View Resume
                </a>
