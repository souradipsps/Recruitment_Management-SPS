...
export default function JobRequestMobileCards({ filteredRequests, onView, scrollRef, currentCardIndex, setCurrentCardIndex, totalItems = filteredRequests.length, startIndex = 0 }) {
  return (
    <div style={{ marginBottom: 4 }}>
      <div style={{ fontSize: 12, color: T.inkFaint, fontWeight: 600, marginBottom: 8, textAlign: "center" }}>
        Showing {totalItems > 0 ? startIndex + 1 : 0} - {Math.min(startIndex + filteredRequests.length, totalItems)} of {totalItems} request{totalItems !== 1 ? "s" : ""}
      </div>
