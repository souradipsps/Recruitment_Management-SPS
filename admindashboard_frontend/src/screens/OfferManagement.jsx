...
export default function OfferManagement({ offers, setOffers, jobPostings = [] }) {
  ...
  const displayFilteredOffers = filteredOffers.slice(startIndex, endIndex);
            ...
            <>
              ...
              >
                ...
                {enrichedPostings.map((p, idx) => {
                  ...
                  return (
                    <div key={p.id} onClick={() => { selectPosting(p.id); setFilterActiveIndex(idx + 1); if (hScroll.ref.current) { const cards = hScroll.ref.current.children; if (cards[idx + 1]) cards[idx + 1].scrollIntoView({ behavior: "smooth", block: "nearest", inline: "center" }); } }} style={{ flexShrink: 0, width: "100%", border: `2px solid ${isSelected ? accentColor : T.border}`, borderRadius: 16, padding: "18px 20px", cursor: "pointer", background: isSelected ? accentPale : T.surface, transition: "all 0.2s", boxShadow: isSelected ? `0 4px 20px ${accentColor}22` : "0 1px 4px rgba(0,0,0,0.05)" }}>
      ...
      {isMobile ? (
        <div style={{ marginBottom: 4 }}>
          <div style={{ fontSize: 12, color: T.inkFaint, fontWeight: 600, marginBottom: 8, textAlign: "center" }}>
            Showing {totalItems > 0 ? startIndex + 1 : 0} - {Math.min(startIndex + displayFilteredOffers.length, totalItems)} of {totalItems} offers
          </div>
          ...
          <div ref={scrollRef} onScroll={(e) => { const scrollLeft = e.currentTarget.scrollLeft; const cardWidth = e.currentTarget.clientWidth; const newIndex = Math.round(scrollLeft / cardWidth); setCurrentCardIndex(newIndex); }} style={{ display: "flex", overflowX: "auto", WebkitOverflowScrolling: "touch", scrollbarWidth: "none", msOverflowStyle: "none", gap: 16, padding: "0 16px 20px", margin: "0 -16px" }}>
            {displayFilteredOffers.map((o, idx) => {
        ...
        <Card>
          <div style={{ padding: "12px 14px", borderBottom: `1px solid ${T.border}`, display: "flex", justifyContent: "flex-end" }}>
            <span style={{ fontSize: 12, color: T.inkFaint, fontWeight: 600, whiteSpace: "nowrap" }}>{totalItems > 0 ? startIndex + 1 : 0} - {Math.min(endIndex, totalItems)} of {totalItems} offers</span>
          </div>
          <Table
            cols={["Offer ID", "Candidate", "Role", "Score", "Status", "Generate", "Actions"]}
            onRowClick={(i) => setSelectedOfferForModal(displayFilteredOffers[i])}
            rows={displayFilteredOffers.map((o) => {
              const interview = INTERVIEWS.find(inv => inv.candidate === o.candidate && inv.role === o.role);
